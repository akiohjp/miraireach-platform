...
async function getUnsplashImage(unsplash, query, index) {
  try {
...
const TEMPLATES = [
  ...
  },
  {
    ...
    image_query: "dubai desert luxury",
  ...
  },
  {
    ...
    image_query: "dubai mall architecture",
...
async function main() {
  ...
  const unsplash = unsplashKey ? createApi({ accessKey: unsplashKey }) : null;

  console.log("Generating 30 curated lifestyle and dining articles...");
  const newArticles = [];
  const now = Date.now();
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const imageUrl = await getUnsplashImage(unsplash, template.image_query, i);
  ...
  console.log("Appending 30 curated articles to Supabase...");
  try {
    const inserted = await insertArticles(url, key, newArticles);
    console.log(`Successfully restored ${inserted.length} high-density curated articles!`);
  } catch (err) {
