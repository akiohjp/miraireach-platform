...
async function main() {
  ...
  const newArticles = [];
  const now = Date.now();
  const msInDay = 2 * 60 * 60 * 1000; // Offset by 2 hours each to spread them

  for (let i = 0; i < 30; i++) {
    const template = TEMPLATES[i % TEMPLATES.length];
    const source = SOURCES[i % SOURCES.length];
    
    // Add variation to title and content based on index
  ...
  try {
    ...
    console.log(`Successfully appended ${inserted.length} high-density curated articles!`);
  } catch (err) {
