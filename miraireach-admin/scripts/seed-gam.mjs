/**
 * Seed GAM HQ demo data (organizations, categories, budgets, ledger, projects).
 *
 * Usage:
 *   SUPABASE_SERVICE_ROLE_KEY=... node scripts/seed-gam.mjs <auth_user_uuid>
 *
 * Reads NEXT_PUBLIC_SUPABASE_URL from `.env.local` (project root) if present.
 * The `<auth_user_uuid>` must be a real `auth.users.id` — the same account you use in the app
 * so RLS allows `organization_members` and data reads.
 *
...
if (!userId || !/^[0-9a-f-]{36}$/i.test(userId)) {
  console.error("Usage: node scripts/seed-gam.mjs <auth_user_uuid>");
  process.exit(1);
}
...
async function main() {
  ...
  console.log("Linked user to organization (owner).");
  ...
  if (bIns) throw bIns;
  console.log("Inserted monthly budgets (org-level, JPY minor).");
  ...
  if (insL) throw insL;
  console.log(`Inserted ${rows.length} ledger lines.`);

  console.log("\nDone. Set in .env.local (optional):\n  NEXT_PUBLIC_GAM_ORGANIZATION_ID=" + orgId);
}
