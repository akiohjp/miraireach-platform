...
/** 漫画の原寸（ファイルと一致させる） */
const MANGA_WIDTH = 721;
const MANGA_HEIGHT = 1024;

const MANGA_PAGES = [
  {
    src: "/miraireach/manga-01.jpg",
...
/** Retina では原寸を超えて表示しない（ぼかし防止） */
function useNativePixelWidth(nativeWidth: number) {
  const [cssWidth, setCssWidth] = useState(nativeWidth);

  useEffect(() => {
    const update = () => {
      const dpr = window.devicePixelRatio || 1;
      setCssWidth(Math.min(nativeWidth, Math.floor(nativeWidth / dpr)));
    };
    update();
...
function MangaPageImage({ src, alt, priority }: { src: string; alt: string; priority?: boolean }) {
  const displayWidth = useNativePixelWidth(MANGA_WIDTH);

  return (
    <div className="flex justify-center px-2 pb-2 pt-1 md:px-4 md:pb-3">
      {/* 通常の img で再圧縮なし。表示幅は DPR に合わせ 1:1 ピクセル */}
      <img
        src={src}
        alt={alt}
        width={MANGA_WIDTH}
        ...
        style={{ width: displayWidth, imageRendering: "auto" }}
      />
    </div>
  );
}
